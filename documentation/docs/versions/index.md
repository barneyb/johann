...
At the moment, Johann is building up from "nothing" towards "enough". There's still a _lot_ of kludge, but gone are the days of "anything is better than writing assembly."

For now, there only two constraints on versioning:
...
A few things remain before 1.0:

1. A syntactic revision, including function return types.
1. The compilation pipeline itself has two major shortcomings:
    1. typechecks are half during parsing and half during codegen, and
    1. codegen is direct from the parse tree, without an intermediate representation (aka "three address codes").
1. Some way to deal with varargs (for `printf`). `io` is the last bit of non-syscall assembly, as it still can't be implemented in Johann.
